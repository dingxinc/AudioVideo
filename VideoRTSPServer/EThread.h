...
public:
	...
	bool Start() {
		m_bStatus = true;
		m_hThread = (HANDLE)_beginthread(&EThread::ThreadEntry, 0, this);
		if (!IsValid()) {
			m_bStatus = false;
		}
		return m_bStatus;
...
{
public:
	EThreadPool(size_t size) {
		m_threads.resize(size);
		for (size_t i = 0; i < size; i++)
			m_threads[i] = new EThread();
	}
