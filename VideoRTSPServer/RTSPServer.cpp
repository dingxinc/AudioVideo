...
int RTSPServer::threadWorker()
{
	...
	ESocket client = m_socket.Accept(client_addr);     // 负责接收客户端连接
	if (client != INVALID_SOCKET) {
		RTSPSession session(client);                      // 将连接分发至 session
		m_lstSession.PushBack(session);              
		m_pool.DispatchWorker(ThreadWorker(this, (FUNCTYPE)&RTSPServer::ThreadSession));// 线程池派发任务
	}
	return 0;
}
...
{
	/* 相当于在服务器本地打开一个 h.264 的视频数据直接发送给客户端 */
